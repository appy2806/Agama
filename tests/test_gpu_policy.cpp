...
namespace {
#ifdef HAVE_CUDA
// Templated per-potential parity check. Runs evalmanyCar<Serial> and
// evalmanyCar<Cuda> on N Cartesian points, compares element-by-element to
// `tol` (loose, ~1000 ULPs), and cross-checks the first 8 points against
// the existing `pot.value(pos)` virtual to confirm leaf-vs-virtual single-source.
// Returns true on success.
template<class Pot>
bool check_pot_parity(Pot& pot, const char* name,
    const std::vector<coord::PosCar>& in_h, double rel_tol = 1e-13)
{
    using namespace agama;
    const std::size_t N = in_h.size();
    std::vector<double> phi_s(N);
    pot.evalmanyCar(Serial{}, N, in_h.data(), phi_s.data());
    double max_phi = 1e-300;  // scale for relative comparison (Phi can be O(1) or O(1000))
    for(std::size_t i = 0; i < N; ++i)
        max_phi = std::max(max_phi, std::fabs(phi_s[i]));
    double max_virt_err = 0.0;
    for(std::size_t i = 0; i < 8 && i < N; ++i)
        max_virt_err = std::max(max_virt_err, std::fabs(pot.value(in_h[i]) - phi_s[i]));
    device_array<coord::PosCar> d_in(N);
    d_in.from_host(in_h.data(), N);
    device_array<double> d_phi(N);
    pot.evalmanyCar(Cuda{}, N, d_in.data(), d_phi.data());
...
int main() {
    ...
    // Each potential's evalmanyCar<Policy> template is instantiated here for Cuda
    ...
    // for the leaf math — same Phi expression must produce the same value at FP level.
    {
        ...
        bool ok_pots =
            check_pot_parity(plummer, "Plummer",       in_h) &&
            check_pot_parity(iso,     "Isochrone",     in_h) &&
            check_pot_parity(nfw,     "NFW",           in_h) &&
            check_pot_parity(mn,      "MiyamotoNagai", in_h) &&
            check_pot_parity(logp,    "Logarithmic",   in_h) &&
            check_pot_parity(harm,    "Harmonic",      in_h);
        if(!ok_pots) {
