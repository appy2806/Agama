...
/** (Re-)initialize the persistent state of the Hermite integrator from the given state vector.
    \param[in]  force1  is the first-order r.h.s. of the ODE (Force callable),
    used only for the initial timestep estimate via ode_init_timestep;
    \param[in]  NDIM    is the full size of the ODE system (2 * numVar);
    \param[in]  stateNew  is the new state vector (NDIM elements: positions then velocities);
    \param[in]  accRel  is the tolerance parameter (already rescaled by the class constructor);
    \param[out] state   is the persistent 5*NDIM storage; stateNew is copied into offset 2*NDIM;
    \param[in,out] nextTimeStep  is the prediction for the next timestep:
    if zero on input (the very first call), it is estimated by ode_init_timestep;
    \param[in,out] scratch  is a caller-provided working array of 4*NDIM elements.
*/
template<typename T, typename Force1>
AGAMA_DEVICE_INLINE void hermite_init(const Force1& force1, int NDIM, const T stateNew[],
    T accRel, T* state, T& nextTimeStep, T* scratch)
{
    for(int d=0; d<NDIM; d++)  // copy the vector x
        state[d+NDIM*2] = stateNew[d];
    if(nextTimeStep == T(0))  // initial timestep assignment
        nextTimeStep = ode_init_timestep(force1, NDIM, stateNew, T(0),
            std::pow(accRel, T(8)), scratch);
}

/** Perform a single adaptive timestep of the Hermite integrator
    (4th order Hermite scheme with two force evaluations per timestep).
    \param[in]  force2  is the second-order r.h.s. of the ODE (Force2 callable, providing
    d2x/dt2 and d3x/dt3);
    \param[in]  NDIM    is the full size of the ODE system (2 * numVar);
    \param[in]  accRel  is the relative tolerance parameter (already rescaled);
    \param[in,out] state  is the persistent 5*NDIM storage: on entry, state[2*NDIM..3*NDIM-1]
    hold the full state (positions then velocities) as deposited by hermite_init or the previous
    call; on successful return, the full 5*NDIM block is updated with the completed step and the
    dense-output coefficients (snap, crackle);
    \param[in,out] nextTimeStep  is the prediction for the next timestep, updated on return;
    \param[in]  maxTimeStep  is the upper limit on the length of the timestep.
    \return  the (signed) length of the timestep taken, or 0 on error.
*/
template<typename T, typename Force2>
AGAMA_DEVICE_INLINE T hermite_step(const Force2& force2, int NDIM, T accRel,
    T* state, T& nextTimeStep, T maxTimeStep)
{
    if(maxTimeStep == T(0))
        return T(0);
    int numVar = NDIM / 2;
    ...
    force2(T(0), oldpos, /*output*/ oldacc, oldjrk, (T*)NULL);
    // use the previously estimated timestep or the requested max step, whichever is shorter
    T sign = maxTimeStep >= T(0) ? T(+1) : T(-1), dt;
    nextTimeStep = std::fmin(maxTimeStep * sign, nextTimeStep);
    int nreject = 0;
    do {
        dt = nextTimeStep * sign;
        // predict the coordinates/velocities at the end of timestep
        for(int i=0; i<numVar; i++) {
            newpos[i] = oldpos[i] + dt * (oldvel[i] + T(0.5) * dt * (oldacc[i] + T(1./3) * dt * oldjrk[i]));
            newvel[i] = oldvel[i] + dt * (oldacc[i] + T(0.5) * dt *  oldjrk[i]);
        }
        // compute acceleration and jerk at the end of step, using predicted pos/vel
        force2(dt, newpos, /*output*/ newacc, newjrk, (T*)NULL);
        T sumasq=T(0), sumjsq=T(0), sumssq=T(0), sumcsq=T(0);
        for(int i=0; i<numVar; i++) {
        ...
        nextTimeStep = accRel * std::sqrt((std::sqrt(sumasq * sumssq) + sumjsq) / (std::sqrt(sumjsq * sumcsq) + sumssq));
        // repeat if new estimate of timestep was substantially less than the actually used one
    } while(nextTimeStep < T(0.5) * dt * sign && ++nreject < 3);
    ...
    for(int i=0; i<numVar; i++) {
        ...
        newvel[i] = oldvel[i] + T(0.5) * dt * (oldacc[i] + newacc[i] + T(1./6) * dt * (oldjrk[i] - newjrk[i]));
        newpos[i] = oldpos[i] + T(0.5) * dt * (oldvel[i] + newvel[i] + T(1./6) * dt * (oldacc[i] - newacc[i]));
    }
    return dt;
}

/** Dense-output interpolation of the solution within the last completed Hermite timestep.
    \param[in]  state   is the persistent 5*NDIM storage of the integrator (numVar = NDIM/2),
    with the dense-output coefficients filled by the last successful hermite_step;
    \param[in]  NDIM    is the full size of the ODE system (2 * numVar);
    \param[in]  prevTimeStep  is the signed length of the last completed timestep;
    \param[in]  timeOffset  is the time offset from the beginning of that timestep;
    \param[in]  ind  is the index of the requested component (0..NDIM-1).
...
*/
template<typename T>
AGAMA_DEVICE_INLINE T hermite_dense(const T* state, int NDIM, T prevTimeStep, T timeOffset, int ind)
{
    int numVar = NDIM / 2;
    int i = ind % numVar;
    if(ind < numVar) {  // interpolate position
        return                     state[i         ] +
            ...
            timeOffset * T(1./2) * (state[i+2*numVar] +
            timeOffset * T(1./3) * (state[i+3*numVar] +
            timeOffset * T(1./4) * (state[i+8*numVar] +
            timeOffset * T(1./5) *  state[i+9*numVar]))));
    } else {  // interpolate velocity
        return                     state[i+  numVar] +
            timeOffset *          (state[i+2*numVar] +
            timeOffset * T(1./2) * (state[i+3*numVar] +
            timeOffset * T(1./3) * (state[i+8*numVar] +
            timeOffset * T(1./4) *  state[i+9*numVar])));
    }
    (void)prevTimeStep;
}
