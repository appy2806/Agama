...
/** cross-coord gradient/hessian transforms for the Car/Cyl/Sph triangle — bodies
    promoted from coord.cpp so they are callable from CUDA kernels: every branch of
    Multipole::evalCyl routes through toGrad<Sph,Cyl>/toHess<Sph,Cyl> (via
    transformDerivsSphToCyl in potential_multipole.cpp), so a device kernel cannot
    evaluate a Multipole without these. The ProlSph/Axi specializations deliberately
    stay host-only in coord.cpp: they serve the Stackel action finder and the focal-
    distance finder, both of which stay CPU forever (iterative, wrong shape for SIMT). */

template<> AGAMA_DEVICE_INLINE
